...
function transfrom(tmpl) {
    function unescape(code) {
        return code.replace(/\\('|\\)/g, "$1").replace(/[\r\t\n]/g, " ");
    }
    var c = {
        func :       /(\/\* def viper-template \*\/)([\s\S]*?)(\/\* end \*\/)/g,
        evaluate:    /\{\{([\s\S]+?(\}?)+)\}\}/g,
        interpolate: /\{\{=([\s\S]+?)\}\}/g,
        conditional: /\{\{\?(\?)?\s*([\s\S]*?)\s*\}\}/g,
        transclude:  /\{\{\-(\-)?\s*([\s\S]*?)\s*\}\}/g,
    ...
    var match = tmpl.match(c.iterate) || [];
    var ret = match.map(function(str) {
        return match[0].replace(/.*\(|\).*|\s+/g, '');
    }).map(function(str) {
        return str.split('/');
    }).reduce(function(mem, item) {
        item.shift();
        return union(mem, item);
    }, []);

    var disable = '/* eslint-disable no-unused-vars, semi, indent */';
    var enable = '/* eslint-enable no-unused-vars, semi, indent */';
    ret = ret.concat(['$if', '$else', '$elif', '$fi', '$each', '$end']);

    // 只替换{{#func()}}{{#}}里面的内容
    tmpl = tmpl.replace(c.func, function(m, start, str, end) {
        // 转译特殊字符，在html拼接的时候出现语法错误
        // console.log(start, end, str);
        start = start + disable + 'var ' + ret.join(', ') + ';';
        end = enable + end;
